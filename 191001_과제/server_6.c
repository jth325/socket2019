...
int main()
{
	...
	while (1)
	{
		...
		else if (strncmp(rcvBuffer, "strlen", 6) == 0 && n >= 8)
		{
			sprintf(sndBuffer, "문자열 길이 : %d", strlen(rcvBuffer + strlen("strlen") + 1));
			write(c_socket, sndBuffer, strlen(sndBuffer));
		}
		else if (strncmp(rcvBuffer, "strcmp", 6) == 0)
		{
			...
			sprintf(sndBuffer, "비교 결과 : %d", strcmp(str1, str2));
			write(c_socket, sndBuffer, strlen(sndBuffer));
		}
		else if (strncmp(rcvBuffer, "readfile", 8) == 0 && n >= 10)
		{
			fp = fopen(rcvBuffer + 9, "r");
			while (fgets(sndBuffer, sizeof(sndBuffer), fp))
				write(c_socket, sndBuffer, strlen(sndBuffer));
			
		}
		else if (strncmp(rcvBuffer, "exec", 4) == 0 && n >= 6)
		{
			system(rcvBuffer + 5);
		}
